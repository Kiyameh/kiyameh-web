...
import type { Project, ProjectCategory } from "@/content/config";
import Button from "@/components/ui/Button/Button";
import filterProjectsByType from "@/lib/filterProyectByType";
import {
	Asterisk,
	Globe,
	Wrench,
...
export default function ProjectHighlighter({
	...
	const filterProjects = (category: AvailableCategories) => {
		...
		const filtered = filterProjectsByType(
			projects,
				...
				{filteredProjects.map((project) => (
					<li key={project.id} className={styles.projectItem}>
						<a href={`projects/${project.data.slug}`} key={project.id}>
							...
							<Grapper mode="ghost" variant="sticky">
